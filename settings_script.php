...
if ($new_pass != $new_pass1) {
            $msg .= "<span class='red'><b>The two passwords dosen't match'<b></span><br>";
        header("Location: settings.php?msg=" .$msg);
} else {
    if ($old_pass == $orig_pass) {
        $query = "UPDATE  users SET password = '" . $new_pass . "' WHERE email = '" . $_SESSION['email'] . "'";
        mysqli_query($con, $query) or die($mysqli_error($con));
        $msg .= "<span class='red'>Password Updated</span><br>";
        header("Location: settings.php?msg=" .$msg);
    } else
        $msg .= "<span class='red'>Wrong Old Password</span><br>";
        header("Location: settings.php?msg=" .$msg);
}
